...
export const validWithdraw = (body: any) => {
  ...
  if (body.asset_code !== Config.ASSETS_CODE) {
    return false;
  }
  if (body.asset_issuer !== Config.ASSETS_ISSUER) {
    return false;
  }
...
export const bridgePay = async (
  ...
  const options = {
    ...
    formData: {
      ...
      amount,
      asset_code: Config.ASSETS_CODE,
      asset_issuer: Config.ASSETS_ISSUER
    },
    json: true,
