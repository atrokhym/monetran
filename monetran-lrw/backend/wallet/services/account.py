...
def get_account_balance_by_wyre_wallet(wyre_wallet_id, wyre_user_id):
    account_balance = Balance()
    api = WyreApi()
    response = api.getWallet(wyre_wallet_id, wyre_user_id)
    if response:
        balances = response["balances"]
        for currency, dec_amount in balances.items():
            quote_currency = Currency[currency]
            if quote_currency not in account_balance.total:
                account_balance.total[quote_currency] = 0
            account_balance.total[quote_currency] += (
