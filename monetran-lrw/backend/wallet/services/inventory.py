...
def setup_inventory_account():
    ...
    inventory_account = create_account(account_name=INVENTORY_ACCOUNT_NAME)
    ...
    setup_usdc_inventory_account(inventory_account)


def setup_usdc_inventory_account(inventory_account) -> None:
    internal_address = get_inventory_deposit_address()

    destination_id = inventory_account.id
    destination_subaddr = inventory_account.subaddresses[-1].address
    ...
    storage.add_transaction(
        ...
        currency=DiemCurrency.USDC,
        payment_type=TransactionType.INTERNAL,
        status=TransactionStatus.COMPLETED,
        destination_id=destination_id,
        destination_subaddress=destination_subaddr,
        sequence=0,
    )
