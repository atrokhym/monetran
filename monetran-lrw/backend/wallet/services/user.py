...
from wallet.services.wyre import WyreApi
...
def update_user(
    ...
    # call wyre createUser
    if first_name and last_name and phone and address_1 and city and zip and country:
        ...
        api = WyreApi()
        response = api.createUser(userpayload)
        if response:
            wyre_user_id = response["id"]
            response = api.createWallet(wyre_user_id)
            if response:
                wyre_wallet_id = response["id"]
    ...
    user = storage.update_user(
        ...
        address_2=address_2,
        zip=zip,
        wyre_user_id=wyre_user_id,
        wyre_wallet_id=wyre_wallet_id,
    )
...
def get_user_by_wallet(wyre_wallet_id: str) -> User:
    return storage.get_user_by_wallet(wyre_wallet_id)
...
def add_payment_method(
    user_id: str, name: str, provider: str, token: str, raw: str
) -> None:
    user = User.query.get(user_id)
    wyre_paymentmethod_id = None
    bank_account = json.loads(raw)

    if provider == "BankAccount":
        api = WyreApi()
        response = api.createPaymentMethod(user, bank_account)
        if response:
            wyre_paymentmethod_id = response["id"]

    storage.add_user_payment_method(
        user_id, name, provider, token, raw, wyre_paymentmethod_id
    )
