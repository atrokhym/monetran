...
def get_user_by_wallet(wyre_wallet_id: str) -> User:
    return User.query.filter_by(wyre_wallet_id=wyre_wallet_id).first()
...
def update_user(
    ...
    wyre_user_id: Optional[str] = None,
    wyre_wallet_id: Optional[str] = None,
) -> User:
    user = User.query.get(user_id)
    if user is not None:
    ...
    if user is not None:
        user.payment_methods.append(
            PaymentMethod(
                ...
                token=token,
                raw=raw,
                wyre_paymentmethod_id=wyre_paymentmethod_id,
            )
        )
        db_session.add(user)
        db_session.commit()
