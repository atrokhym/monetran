    ...
    const refreshUser = async () => {
      try {
        if (SessionStorage.getAccessToken()) {
          ...
          const fiatCurrencies = { ...settings.fiatCurrencies };
          for (const rate of rates) {
            const [currency, fiatCurrency] = rate.currency_pair.split("_");
            if (fiatCurrencies[currency])
              fiatCurrencies[currency].rates[fiatCurrency] = rate.price;
          }

          const sortedBalances = account.balances.sort((a, b) => {
            const currencyA = settings.currencies[a.currency] ? settings.currencies[a.currency] : settings.fiatCurrencies[a.currency];
            const currencyB = settings.currencies[b.currency] ? settings.currencies[b.currency] : settings.fiatCurrencies[b.currency];

            const exchangeRateA = currencyA.rates ? currencyA.rates[settings.defaultFiatCurrencyCode!] : 1;
            const priceA = diemAmountToFloat(a.balance) * exchangeRateA;
            const exchangeRateB = currencyB.rates ? currencyB.rates[settings.defaultFiatCurrencyCode!] : 1;
