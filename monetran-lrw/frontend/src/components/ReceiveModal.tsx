...
import { ADDR_PROTOCOL_PREFIX, WYRE_ADDR_PROTOCOL_PREFIX } from "../interfaces/blockchain";
...
function ReceiveModal({ open, onClose, currency }: ReceiveModalProps) {
  ...
  async function fetchReceivingAddress(currency: DiemCurrency) {
    try {
      ...
      setReceivingAddressWithIntents(`${WYRE_ADDR_PROTOCOL_PREFIX}${recvAddress}?c=${currency}`);
