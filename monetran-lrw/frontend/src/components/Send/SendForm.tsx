...
import { WYRE_ADDR_PROTOCOL_PREFIX, ADDR_PROTOCOL_PREFIX, VALID_WYRE_ADDRESS_REGEX } from "../../interfaces/blockchain";
...
function parseAddressIntents(address: string): AddressWithIntents {
  ...
  if (address.startsWith(WYRE_ADDR_PROTOCOL_PREFIX)) {
    address = address.substring(WYRE_ADDR_PROTOCOL_PREFIX.length);
  }
      ...
      <FormGroup>
        ...
        <Input
          ...
          innerRef={register({
            ...
            pattern: {
              value: VALID_WYRE_ADDRESS_REGEX,
              message: (
